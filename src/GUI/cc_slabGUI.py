...
relative_path = "..\input\CC_slab.txt"

# Combine the current directory with the relative path to create the full file path
cc_slab_file = os.path.join(current_dir, relative_path)
...
class CCSlabParametersWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super(CCSlabParametersWindow, self).__init__()
        self.setWindowTitle("CC_slab")
...
# You can add additional code to handle other functionalities specific to the CC_slab GUI if needed

if __name__ == "__main__":
    app = QtWidgets.QApplication([])
    window = CCSlabParametersWindow()
    window.show()
    app.exec_()
